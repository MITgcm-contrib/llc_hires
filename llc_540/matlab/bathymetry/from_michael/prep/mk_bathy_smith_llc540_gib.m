...
for tl=6:6 % 1:length(td)
 ...
 if tile < 10
  fid1 =fopen(['runtime_smith_llc540_tile0' num2str(tile) '_gz' num2str(ztd) '_t' num2str(i) '.out'],'w','b');
 else
  fid1 =fopen(['runtime_smith_llc540_tile' num2str(tile) '_gz' num2str(ztd) '_t' num2str(i) '.out'],'w','b');
 end
...
for SLICE=b1:bi:b2
 ...
 for it=1:length(IX), mydisp(it)
   ...
   if iy
     TOPO01(IX(it))=mynanmedian(double(bat1(ix(iy))));
   else
     TOPO01(IX(it))=0;
  end  % if iy
 end % for it
 ...
 if tile < 10
  eval(['save ' pout_t 'tmp540_smith_bathy_td0' num2str(tile) '_gz' num2str(ztd) '_t' num2str(i) ' TOPO01 -v7.3'])
 else
  eval(['save ' pout_t 'tmp540_smith_bathy_td' num2str(tile) '_gz' num2str(ztd) '_t' num2str(i) ' TOPO01 -v7.3'])
 end
