  ...
  int  writeFlag)
{
    ...
    if (writeFlag) {
        ...
        MPI_IOc = MPI_File_write_all;
    } else {
        fileFlags = MPI_MODE_RDONLY;
        MPI_IOc = MPI_File_read_all;
    }
    ...
    // MPI_IOc(fh, tileBuf, 1, tileLayoutInMemory, MPI_STATUS_IGNORE);
    res = MPI_IOc(fh, tileBuf, 1, tileLayoutInMemory, &status);
