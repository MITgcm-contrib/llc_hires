...
%    Theta=readbin(fnm,[nx ny]);
%    minTheta=min(minTheta,mmin(Theta)); maxTheta=max(maxTheta,mmax(Theta));
%end
%cx=[minTheta maxTheta];

cx1=[15.5 25.5]; cx2=[-1 1];
clf reset; colormap(cmap)
meanSST=nan*ones(106,9); stdSST=meanSST;
for ts=0:3456:366192
    clf
    dte=datestr(datenum('Mar01 2012')+ts*25/60/60/24,'yy/mm/dd:hh');

    fnm=['../Theta/' myint2str(ts+597888,10) '_Theta_11089.9208.1_288.468.88'];
    if exist(fnm), Theta1=readbin(fnm,[nx ny]); else Theta1=Theta1*nan; end
    meanSST(ts/3456+1,1)=mean(Theta1(:)); stdSST(ts/3456+1,1)=std(Theta1(:));
    ...
    if exist(fnm), Theta2=readbin(fnm,[nx ny]); else Theta2=Theta2*nan; end
    meanSST(ts/3456+1,2)=mean(Theta2(:)); stdSST(ts/3456+1,2)=std(Theta2(:));
    ...
    if exist(fnm), Theta3=readbin(fnm,[nx ny]); else Theta3=Theta3*nan; end
    meanSST(ts/3456+1,3)=mean(Theta3(:)); stdSST(ts/3456+1,3)=std(Theta3(:));
    ...
    if exist(fnm), Theta4=readbin(fnm,[NX NY]); else Theta4=Theta4*nan; end
    meanSST(ts/3456+1,4)=mean(Theta4(:)); stdSST(ts/3456+1,4)=std(Theta4(:));
    ...
    if exist(fnm), Theta5=readbin(fnm,[NX NY]); else Theta5=Theta5*nan; end
    meanSST(ts/3456+1,5)=mean(Theta5(:)); stdSST(ts/3456+1,5)=std(Theta5(:));
    ...
    subplot(335), tmp=Theta2'-Theta1';
    meanSST(ts/3456+1,6)=mean(tmp(:)); stdSST(ts/3456+1,6)=std(tmp(:));
    mypcolor(tmp'); set(gca,'xtick',[],'ytick',[]);
    caxis(cx2); colorbar; title('2km:88l - global')

    subplot(336), tmp=Theta3'-Theta2';
    meanSST(ts/3456+1,7)=mean(tmp(:)); stdSST(ts/3456+1,7)=std(tmp(:));
    mypcolor(tmp'); set(gca,'xtick',[],'ytick',[]);
    caxis(cx2); colorbar; title('2km:264l - 2km:88l')

    subplot(338), tmp=Theta4a'-Theta2';
    meanSST(ts/3456+1,8)=mean(tmp(:)); stdSST(ts/3456+1,8)=std(tmp(:));
    mypcolor(tmp'); set(gca,'xtick',[],'ytick',[]);
    caxis(cx2); colorbar; title('250m:88l - 2km:88l')

    subplot(339), tmp=Theta5a'-Theta2';
    meanSST(ts/3456+1,9)=mean(tmp(:)); stdSST(ts/3456+1,9)=std(tmp(:));
    mypcolor(tmp'); set(gca,'xtick',[],'ytick',[]);
    caxis(cx2); colorbar; title('250m:264l - 2km:88l')

    ax=axes('Units','Normal','Position',[0 0 1 1],'Visible','off');
    set(get(ax,'Title'),'Visible','on')
    text(.5,1,'SST (dec C) for 24-32N, 193-199E', ...
         'HorizontalAlignment','center', ...
         'VerticalAlignment','top','fontsize',18,'fontweight','bold');
    
    eval(['print -djpeg SST/frame' myint2str(ts/3456,4)])
    save SST/stats meanSST stdSST
end
